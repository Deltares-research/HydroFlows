...
def test_rule_to_dict(rule):
    ...
    assert rule_dict["kwargs"] == {'input_file1': 'test_file1', 'input_file2': 'test_file2', 'param': 'param'}
    assert rule_dict["rule_id"] == "test_rule"
    
def test_detect_wildcards(workflow):
    expand_method = MockExpandMethod(input_file="{region}/test_file", root="{region}", events=["1", "2", "3"], wildcard="w")
    rule = Rule(method=expand_method, workflow=workflow, rule_id="test_rule")
    assert rule._wildcards == {'explode': ['region'], 'expand': ['w'], 'reduce': []}
    assert rule._wildcard_fields == {'region': ['input_file', 'output_file', 'output_file2', 'root'], 'w': ['output_file', 'output_file2']}
    
    reduce_method = MockReduceMethod(first_file=["test1_{w}","test_2{w}"], second_file=["test1_{w}", "test2_{w}"], root="/")
...
def test_validate_wildcards(workflow, test_method):
    ...
    with pytest.raises(ValueError, match=re.escape(f"wildcard(s) {rule.wildcards["expand"]} missing on inputs {test_method.dict["input"]}")):
        rule._validate_wildcards()
    expand_method = MockExpandMethod(input_file="test_file", root="", events=["1", "2", "3"], wildcard="w")
    expand_method.output = ExpandMethodOutput(output_file="test", output_file2="test2")
    with pytest.raises(ValueError, match=re.escape(f"wildcard(s) missing on outputs {expand_method.dict["output"]}")):
        rule = Rule(method=expand_method, workflow=workflow, rule_id="test_rule")
