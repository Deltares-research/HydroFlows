...
def test_expand_wildcard_product():
    method = MockExpandMethod(input_file="test", root=".", events=["1", "2"])
    assert method._expand_wildcards == {"wildcard": ["1", "2"]}
    assert len(method.expand_wildcard_product) == 2
    assert method.expand_wildcard_product[0] == {"wildcard": "1"}
    ...
    assert len(method.expand_wildcard_product) == 4
    assert method.expand_wildcard_product[0] == {"wildcard1": "1", "wildcard2": "3"}


def test_iter_wildcard_output():
    method = MockExpandMethod(input_file="test", root="root_dir", events=["1", "2"])
    for output in method.iter_wildcard_output():
        assert output[0] == {"wildcard": "1"}
        assert output[1] == {
    ...
    for output in method.iter_wildcard_output():
        assert output[0] == {"wildcard1": "1", "wildcard2": "3"}
        assert output[1] == {
