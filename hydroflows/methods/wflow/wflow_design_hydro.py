...
class Input(BaseModel):
    ...
    required for the WflowDesignHydro method.
...
class Output(BaseModel):
    ...
    generated by the WflowDesignHydro method.
    """
...
class Params(BaseModel):
    ...
    in the `WflowDesignHydro` method to define the required settings.
    """

    # parameters for the get_peaks function
    ev_type: str = Field(
        "BM",
        description="Event type. Valid options are 'BM' for block maxima or 'POT' for Peak over threshold.",
        regex="^(BM|POT)$",
    )
    min_dist_days: int = Field(7, description="Minimum distance in days")
    qthresh: float = Field(0.95, description="Quantile threshold")
    min_sample_perc: int = Field(80, description="Minimum sample percentage")
    index_dim: str = Field("Q_gauges", description="Index dimension")
    time_dim: str = Field("time", description="Time dimension")
    t0: str = Field("2020-01-01", description="Initial date")
    rps: ListOfFloat = Field(
        [1, 2, 5, 10, 20, 50, 100], description="Return periods of interest."
    )
    plot_fig: bool = Field(True, description="Whether to plot the figure or not.")
    wdw_size_days: int = Field(6, description="Duration for hydrograph in days.")
