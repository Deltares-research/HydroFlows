...
class FluvialHistoricalEvents(ExpandMethod):
    ...
    def run(self):
        ...
        if dt.total_seconds() % 86400 == 0:
            unit = "days"
            freq = f"{int(dt.total_seconds() / 86400)}D"
        elif dt.total_seconds() % 3600 == 0:
            unit = "hours"
        ...
        if unit == "days" and any(
            ":" in date
            for event in self.params.events_dates.values()
            for date in event.values()
        ):
            warnings.warn(
                "Event dates contain hours, but the dataset resolution is daily. Hourly details will be ignored.",
                stacklevel=2,
            )
        ...
        for event_name in self.params.events_dates.keys():
            ...
            forcing_file = Path(str(self.params.output.event_csv).format(**fmt_dict))

            event_data = da.sel(time=slice(start_time_event, end_time_event))
            event_data_aligned = event_data.resample({time_dim: freq}).nearest()

            event_data_aligned.to_pandas().round(2).to_csv(forcing_file)
