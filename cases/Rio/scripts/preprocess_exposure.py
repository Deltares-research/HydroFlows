...
data_source = Path(os.path.abspath(__file__)).parent.parent /"setup_data"  / "hydromt_fiat_exposure" # make a relative path
...
def convert_to_2d(geometry):
    if geometry.geom_type == 'Polygon':
        ...
        return MultiPolygon([Polygon([(x, y) for x, y, z in poly.exterior.coords])
                             for poly in geometry.geoms])  # Use geometry.geoms here
    else:
        return geometry
...
occupancy_gdf.drop_duplicates(subset = 'geometry',inplace=True) # NOTE currently no logic behind dropping. Could drop the higher impacts. Discuss with partners

# Set crs
if buildings_gdf.crs != occupancy_gdf.crs:
    occupancy_gdf = gpd.GeoDataFrame(
        occupancy_gdf, geometry="geometry"
...
dic_occupancy = {1: 'Commercial and Service', 2: 'Residential', 3: 'Commercial and Service', 4: 'Commercial and Service', 5: 'Industrial' , 6 : 'Commercial and Service',7 : 'Residential', 8: 'Residential', 9: 'Residential'}
occupancy_gdf["primary_object_type"] = occupancy_gdf['cod_uso'].map(dic_occupancy).fillna('Residential')
del occupancy_gdf['cod_uso']

# JRC Curve Mapping 
# Translate occupancy to JRC Damage Curves  #TODO Decide on occupancy translation
dic_occupancy_to_jrc_damages = {'Commercial and Service': 'commercial' ,'Residential': 'residential','Temples, Churches, etc.': 'commercial', 'Mixed (commercial and residential)': 'commercial', 'Industrial': 'industrial',  'Public': 'commercial', 'Empty Land': 'residential','Abandoned': 'residential', 'Others': 'residential'}
occupancy_gdf_jrc = occupancy_gdf.copy()
occupancy_gdf_jrc["primary_object_type"] = occupancy_gdf["primary_object_type"].map(dic_occupancy_to_jrc_damages).fillna('residential')
...
occupancy_gdf = gpd.sjoin(
    occupancy_gdf, census_gdf, how="left", predicate="within"
)
...
## Save occupancy
occupancy_gdf[['geometry', 'primary_object_type']].to_file((data_source / "occupancy_pre_processed_translated_occupaction_local_dummy.gpkg"))
occupancy_gdf_jrc[['geometry', 'primary_object_type']].to_file((data_source / "occupancy_pre_processed_translated_occupaction_jrc.gpkg"))

# Save Finished Floor Height
##convert from cm into meters
occupancy_gdf["altura"] = occupancy_gdf["altura"]/100
occupancy_gdf[["altura", "geometry"]].to_file(data_source / "finished_floor_height.gpkg")

## Save population 
occupancy_gdf[["residents", "geometry"]].to_file(data_source / "asset_population.gpkg", driver="GPKG")
